...
public partial class _Default : System.Web.UI.Page 
{
    ...
    {
        DataServiceContext ctx = new DataServiceContext(new Uri("http://localhost:2149/Listing16-05/AdventureWorksDataService.svc"));
        //    http://localhost:1428/AdvSearchService/AdventureWorksDataService.svc"));
        DataServiceQuery<Product> qry = ctx.CreateQuery<Product>("/Product?$filter=FinishedGoodsFlag eq true&$orderby=Name asc");
    ...
    protected void ProductDropDownList_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        DataServiceContext ctx = new DataServiceContext(new Uri("http://localhost:2149/Listing16-05/AdventureWorksDataService.svc"));
        //"http://localhost:1428/AdvSearchService/AdventureWorksDataService.svc"));
        DataServiceQuery<Product> qry = ctx.CreateQuery<Product>(string.Format("/Product({0})", id));
        foreach (Product p in qry.Execute())
        {
